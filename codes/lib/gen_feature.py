...
def create_aggregated_features_kernel(train, test, train_active, test_active, 
            train_periods, test_periods, 
            todir, ext):
    print('\n>> create aggregated features')
    filename = todir + 'aggregated-features' + ext
    if os.path.exists(filename):
        print ('done already...')
        gp = load_file(filename, ext)
    else:
        # if DEBUG: 
        #     print(train.head()); print(test.head()); print(train_active.head())
        #     print(test_active.head()); print(train_periods.head()); print(test_periods.head())
        ##---------------------------------------
        ...
        del test_periods
        gc.collect()
        # if DEBUG: print('\nall periods'); print(all_periods.head())

        ##---------------------------------------
        all_periods['days_up'] = (all_periods['date_to'] - all_periods['date_from']).dt.days
        # if DEBUG: print('\nall periods days_up'); print(all_periods.head())
        ...
        # if DEBUG: print('\nall periods days_up_sum, times_put_up'); print(gp_df.head())

        ##---------------------------------------
        all_periods.drop_duplicates(['item_id'], inplace=True)
        all_periods = all_periods.merge(gp_df, on='item_id', how='left')
        # if DEBUG: print('\nall periods merge'); print(all_periods.head())
        del gp, gp_df; gc.collect()

        ##---------------------------------------
        all_periods = all_periods.merge(all_samples, on='item_id', how='left')
        # if DEBUG: print('\nall periods merge on item_id'); print(all_periods.head()); 
        print(all_periods.info())
